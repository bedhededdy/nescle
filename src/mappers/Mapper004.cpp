...
void Mapper004::Reset() {
    ...
    prg_banks[3] = (Cart_GetPrgRomBlocks(cart) * 2 - 1) * 0x2000;
}
...
uint8_t Mapper004::MapCPURead(uint16_t addr) {
    ...
    return Cart_ReadPrgRom(cart, prg_banks[(addr - 0x8000) / 0x2000] + (addr % 0x2000));
}
    ...
    if (addr >= 0x8000 && addr <= 0x9FFF) {
        if (!(addr & 1)) {
            ...
            if (prg_bank_mode) {
                ...
                prg_banks[0] = (Cart_GetPrgRomBlocks(cart) * 2 - 2) * 0x2000;
            } else {
...
uint8_t Mapper004::MapPPURead(uint16_t addr) {
    ...
    return Cart_ReadChrRom(cart, chr_banks[addr / 0x400] + (addr % 0x400));
}

bool Mapper004::MapPPUWrite(uint16_t addr, uint8_t data) {
    if (Cart_GetChrRomBlocks(cart) == 0) {
        Cart_WriteChrRom(cart, chr_banks[addr / 0x400] + (addr % 0x400), data);
        return true;
    }
